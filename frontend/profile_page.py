from fasthtml.common import *
sys.path.append(os.path.join(os.path.dirname(__file__),'..'))
from backend.system import main_system

def profile_page(session):
   user_id = session['auth'][0]
   user = main_system.get_user_by_id(user_id)
   
   order_history = main_system.get_shipping_items(user_id)
   order_history_div = None

   if order_history:
       order_history_div = Div(
    H3('Shipping Status', style='color: #3498DB; font-weight: bold; text-align: center;'),
    Div(
        *[
            Div(
                Div(f'Total Price: {order.get_order.get_total_price} USD', style='color: #1C1C1C; font-weight: bold;'),
                Div(f'Shipping Date: {order.get_shipping_status.get_shipping_date.strftime("%Y-%m-%d %H:%M:%S")}', 
                    style='color: #1C1C1C;'),
                Div(f'Item Received Date: {order.get_shipping_status.get_get_item_date.strftime("%Y-%m-%d %H:%M:%S")}', 
                    style='color: #1C1C1C;'),

                Div(
                    *[
                        Div(
                            Img(src=item.get_item.get_image, style="width: 100px; height: auto; border-radius: 5px;"),
                            H3(item.get_item.get_name, style="text-align: center; font-size: 1rem; color: #1C1C1C;"),
                            style='display: flex; flex-direction: column; align-items: center; padding: 5px;'
                        )
                        for item in order.get_order.get_list_item_select
                    ],
                    style='display: flex; flex-direction: row; gap: 10px; overflow-x: auto; padding: 10px;'
                ),
                style='border-bottom: solid 1px #85C1E9; padding: 10px 0; display: flex; flex-direction: column; gap: 5px; background: #D6EAF8; border-radius: 8px;'
            )
            for order in order_history
        ],
        style='display: flex; flex-direction: column; gap: 15px; padding: 15px; background: #D6EAF8; border-radius: 10px; box-shadow: 0 2px 5px rgba(0, 0, 0, 0.1); width: clamp(500px, 100%, 700px); margin: 20px auto;'
    ),
    style='display: flex; flex-direction: column; justify-content: center; align-items: center; background: #ECF5FF; padding: 20px; border-radius: 10px;'
)

   
   return Main(
    Div(
        Div(
            H2(f'Profile', style='color: #3498DB; text-shadow: 0 0 10px rgba(0, 0, 0, 0.2);'),
            Div(
                Div(
                    Img(
                        src='data:image/png;base64,',
                        style='border-radius: 50%; width: 60px; height: 60px; margin-right: 15px;'
                    ),
                    H2(f'{user.get_username}', style='color: #3498DB;'),
                    style="display: flex; align-items: center; gap: 10px;"
                ),
                P(f'Address: {user.get_address}', style='color: #1C1C1C;'),
                P(f'Birth Date: {user.get_birth_date}', style='color: #1C1C1C;'),
                P(f'Email: {user.get_email}', style='color: #1C1C1C;'),
                P(f'Phone Number: {user.get_phone_number}', style='color: #1C1C1C;'),
                P(f'Username: {user.get_name}', style='color: #1C1C1C;'),
                style="display: flex; flex-direction: column; gap: 10px; padding: 20px; background: #D6EAF8; border-radius: 10px; box-shadow: 0 2px 5px rgba(0, 0, 0, 0.1); width: clamp(600px, 100%, 700px);"
            ),
            style="margin: 20px auto; display: flex; flex-direction: column; gap: 20px; justify-content: center; align-items: center;"
        ),
        order_history_div
    )
)

